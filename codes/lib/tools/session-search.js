...
import { DEFAULT_AGENT_ID, findAgentOrDefault } from "../agents-store.js";
...
export const sessionSearchToolDefinitions = [
    {
        ...
        function: {
            name: "session_search",
            description:
                "Search every archived session transcript (all agents) by keyword or regex. Returns matching user/assistant lines with session file paths — follow up with file_read on a hit for full context. Only conversation text is searched, not tool outputs.",
            parameters: {
                type: "object",
                properties: {
                    query: { type: "string", description: "Case-insensitive keyword, or a regex if it compiles" },
                    agent: { type: "string", description: "Limit to one agent by id or name (default: all)" },
...
export async function executeSessionSearchTool(name, args) {
    ...
    // 세션 루트는 채팅 단위 — 에이전트 필터는 세션 키 소유자로 맞춘다.
    let roots = listSessionRoots();
    if (args?.agent) {
        const found = findAgentOrDefault(args.agent);
        if (!found) return { error: `unknown agent: ${args.agent}` };
        const wanted = found.id;
        roots = roots.filter((row) => (agentIdForSessionKey(row.sessionKey) || DEFAULT_AGENT_ID) === wanted);
    }
    ...
    outer: for (const row of roots) {
        ...
        for (const file of files) {
            ...
            for (const line of extractSessionTextLines(data?.turns)) {
                ...
                if (idx === -1) continue;
                matches.push({
                    agent: agentIdForSessionKey(sessionKey) || DEFAULT_AGENT_ID,
                    sessionKey,
