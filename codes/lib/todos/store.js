...
import { agentColor, agentHomeDir, getAgent, listAgents, DEFAULT_AGENT_ID, DEFAULT_AGENT_NAME } from "../agents-store.js";
...
// 메인 에이전트는 스토어에 없는 가상 에이전트 — 존재 여부 확인용 유사 객체로 대체한다.
function agentOrMain(id) {
    if (!id) return null;
    if (id === DEFAULT_AGENT_ID) return { id: DEFAULT_AGENT_ID, name: DEFAULT_AGENT_NAME };
    return getAgent(id);
}

function agentBrief(id) {
    const agent = agentOrMain(id);
...
export function addTodo(input = {}) {
    ...
    if (list !== "user" && !agentOrMain(list)) return { error: "agent_not_found" };
    ...
    if (input.assigneeId && !agentOrMain(input.assigneeId)) return { error: "agent_not_found" };
...
export function updateTodo(id, patch = {}, opts = {}) {
    ...
    if (patch.assigneeId !== undefined && (patch.assigneeId || null) !== item.assigneeId) {
        ...
        if (patch.assigneeId && !agentOrMain(patch.assigneeId)) return { error: "agent_not_found" };
    ...
    if (patch.list != null) {
        ...
        if (next !== "user" && !agentOrMain(next)) return { error: "agent_not_found" };
...
export function approveSuggestion(id, { fallbackTimeZone } = {}) {
    ...
    if (suggestion.kind === "add") {
        ...
        if (item && suggestion.agentId && agentOrMain(suggestion.agentId)) {
...
export function offerHandoff(todoId, { agentId, reason, prompt } = {}) {
    ...
    if (!agentOrMain(agentId)) return { error: "agent_not_found" };
