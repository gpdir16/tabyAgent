...
import { agentIdForSessionKey, activeSessionFilePath, loadChatHistory } from "../agent/chat-history.js";
...
async function runSessionReview({ sessionKey, agentId }) {
    ...
    const ownerId = agentIdForSessionKey(sessionKey) || agentId;
    if (!sessionFile || !fs.existsSync(sessionFile)) return;

    const { runAgent } = await import("../agent/loop.js");
    const result = await runAgent(buildReviewPrompt({ sessionFile, agentName: ownerId }), {
        chatId: sessionKey,
...
export function maybeScheduleSessionReview({ sessionKey, agentId, result, automated = false }) {
    ...
    const minToolCalls = Number.isFinite(cfg.minToolCalls) ? cfg.minToolCalls : 5;
    ...
    scheduleWork("dream", () => runSessionReview({ sessionKey, agentId }), {
        ...
        .catch((err) => console.error("tabyAgent: session review failed:", err?.stack || err))
