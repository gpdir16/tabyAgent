...
function queueSweep(trigger) {
    ...
    scheduleWork("dream", () => runDreamSweep({ trigger }), { sessionKey: "dream:sweep" })
        .catch((err) => console.error(`tabyAgent: ${trigger} dream sweep failed:`, err?.stack || err))
        .finally(() => {
            sweepRunning = false;
        });
}
...
export function startDreamingScheduler() {
    ...
    if (cfg.enabled === false) {
        console.log("tabyAgent: dreaming disabled");
        return;
    }

    const expr = cron.validate(cfg.cron || "") ? cfg.cron : "0 4 * * *";
    if (!cron.validate(cfg.cron || "")) {
        console.warn(`tabyAgent: invalid dreaming.cron "${cfg.cron}", using "0 4 * * *"`);
    }
    ...
    console.log(`tabyAgent: dream sweep scheduled (${expr}, ${tz})`);
}
