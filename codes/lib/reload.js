...
export function formatReloadReport(report, lang = "en") {
    ...
    if (lang === "ko") {
        ...
        if (!connected.length && !failed.length) {
            lines.push("등록된 MCP 서버가 없습니다 (`/app/user/mcp.json`).");
        }
        ...
        lines.push("");
        lines.push("스킬(`/app/user/skills`)은 다음 메시지부터 바로 반영됩니다.");
    } else if (lang === "ja") {
        lines.push("**MCP を再読み込みしました**");
        if (!connected.length && !failed.length) {
            lines.push("MCP サーバーは未登録です (`/app/user/mcp.json`)。");
        }
        for (const s of connected) {
        ...
        lines.push("");
        lines.push("スキル (`/app/user/skills`) は次のメッセージから反映されます。");
    } else {
        lines.push("**MCP reloaded**");
        if (!connected.length && !failed.length) {
            lines.push("No MCP servers in `/app/user/mcp.json`.");
        }
        for (const s of connected) {
        ...
        lines.push("");
        lines.push("Skills under `/app/user/skills` apply on the next message (no reload needed).");
    }
