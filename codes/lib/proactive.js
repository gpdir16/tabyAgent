import { listAgents, mainAgentRef } from "./agents-store.js";
import { scheduleWork } from "./agent-queue.js";
import { getProactiveConfig } from "./self-improvement.js";
import { sessionKeyForAgent } from "./agent-route.js";
...
- **New to the user, not new to you.** The user is one person across all agents — a fact any agent already surfaced, or one already stored in memory, is not news. Never recite stored schedules, holdings, or dates the user already knows. Unsure whether it was already surfaced? session_search first; still unsure, stay silent.
...
- Do not repeat yourself: if you or another agent already surfaced something and nothing changed, stay silent.
...
export function proactiveTick(now = new Date()) {
    ...
    const pending = [];
    // 메인 에이전트는 스토어에 없는 가상 에이전트이므로 명시적으로 앞에 둔다.
    for (const agent of [mainAgentRef(), ...listAgents()]) {
        const sessionKey = sessionKeyForAgent(agent);
        if (!sessionKey || inFlight.has(agent.id)) continue;
        const last = lastRunAt.get(agent.id) || 0;
        if (now.getTime() - last < cfg.intervalMs) continue;
        inFlight.add(agent.id);
        lastRunAt.set(agent.id, now.getTime());
        pending.push(
            scheduleWork("proactive", () => runCheckin({ agent, sessionKey }), { sessionKey, cancellable: true })
                .catch((err) => console.error(`tabyAgent: proactive check-in failed (${agent.id}):`, err?.stack || err))
    ...
    if (!timer) {
        timer = setInterval(() => {
            try {
                ...
                console.error("tabyAgent: proactive tick failed:", err?.stack || err);
            }
        }, TICK_MS);
        timer.unref?.();
    }
    console.log(`tabyAgent: proactive check-in ${getProactiveConfig().enabled ? "scheduled" : "disabled"}`);
}
