---
name: camofox
description: Anti-detection browser automation for tabyAgent using the CamoFox CLI, REST API, and Camoufox stealth engine.
---
...
**Do not pass `--user`.** `terminal_run` already exports `CAMOFOX_CLI_USER` pointing at this agent's own profile, so plain `camofox` commands share one persistent browser profile and its logins across sessions. Passing a different `--user` creates a separate profile with separate cookies — only do it when you deliberately need an isolated identity.
...
- Persistent contexts per user under `{{CAMOFOX_DATA_DIR}}/profiles`; Linux/macOS keep readable default-user names while Windows uses collision-safe hashed internal profile keys
...
```bash
camofox search "camofox browser" --engine github [tabId] [--user <user>]
```
...
Full endpoint map: `references/api-endpoints.md`.
...
- Rotate vault profile master password regularly for long-lived agents.
...
- Protected API endpoints enforce bearer auth when `CAMOFOX_API_KEY` is configured (including cookie import/export, evaluate, tab/session lifecycle, navigation, interaction, downloads, tracing, console, and errors).
