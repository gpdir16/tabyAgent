...
- Write self-contained HTML/CSS/JS files with `file_patch`.
- For interactive visualizations, consider the **`visualization`** skill instead.
- For deployment, use `terminal_run` with Cloudflare Workers (`wrangler deploy --temporary`) or send the HTML file directly with `telegram_send_file`.
- The user views results in Telegram's in-app browser (mobile) — design mobile-first, touch-friendly.
...
- Dark theme preferred (Telegram default)
- Inline all CSS/JS — no external resources (no CDN, no external fonts, no external images)
- Keep under 5 MiB for Cloudflare Workers deploy