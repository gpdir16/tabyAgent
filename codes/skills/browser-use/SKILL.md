...
- Run every `browser-use` command via **`terminal_run`** (one command per call, or chain with `&&` when you do not need intermediate output).
- CLI is **preinstalled** in the image (`browser-use install` + `browser-use doctor` at image build). Do not `pip install` unless doctor fails after an upgrade.
- Docker has no display: use default **headless** mode; do not use `connect` / `--headed` from Telegram.
